...
Object.entries(required).forEach(([key, config]) => {
  ...
  if (!value) {
    ...
    if (config.minLength && value.length < config.minLength) {
      console.error(`❌ ${key}: Too short (${value.length} chars, need ${config.minLength}+)`);
      hasErrors = true;
    }
...
if (envVars['AUTH_URL']) {
  if (envVars['AUTH_URL'].includes('localhost') || envVars['AUTH_URL'].includes('127.0.0.1')) {
...
if (hasErrors) {
  console.error('❌ Configuration has errors. Please fix them before starting the app.\n');
  process.exit(1);
} else {
  console.log('✅ All required environment variables are configured!\n');
  console.log('📝 Next Steps:');
  console.log('   1. If testing in World App, make sure AUTH_URL points to ngrok');
